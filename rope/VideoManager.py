...
class VideoManager:
    def __init__(self):
        ...
        self.process_qs = []
        self.rec_q = {"Thread": [], "FrameNumber": [], "Status": "clear"}
        self.rec_qs = []
        ...
        if self.is_video_loaded == True:
            ...
            if success:
                ...
                if not self.swap:
                    ...
                    temp = [
                        self.swap_video(target_image, self.current_frame, False),
                        self.current_frame,
                    ]  # temp = RGB
        ...
        if self.is_video_loaded == True:
            ...
            if success:
                ...
                if not self.swap:
                    ...
                    temp = [
                        self.swap_video(target_image, self.current_frame, True),
                        self.current_frame,
                    ]  # temp = RGB
    ...
    def play_video(self, command):
        if command == "play":
            ...
            self.sp = cv2.VideoWriter(
                self.temp_file, cv2.VideoWriter_fourcc(*"mp4v"), self.fps, size
            )
            ...
            for item in self.process_qs:
                if item["Status"] == "clear" and self.current_frame < self.video_frame_total:
                    item["Thread"] = threading.Thread(
                        target=self.thread_video_read, args=[self.current_frame]
                    ).start()
                ...
                if self.process_qs[index]["Status"] == "finished":
                    ...
                    if len(self.fps_average) >= floor(self.fps):
                        ...
                        msg = "%s fps, %s process time" % (
                            fps,
                            round(self.process_qs[index]["ThreadTime"], 4),
                        )
                        self.add_action("send_msg", msg)
                        self.fps_average = []

                    if (
                        self.process_qs[index]["FrameNumber"] >= self.video_frame_total - 1
            ...
            if index != -1:
                ...
                if self.process_qs[index]["Status"] == "finished":
                    ...
                    if (
                        ...
                        self.play_video("stop")
                        stop_time = float(
                            self.capture.get(cv2.CAP_PROP_POS_FRAMES) / float(self.fps)
                        )
                        if stop_time == 0:
    ...
    def thread_video_read(self, frame_number):
        ...
        if success:
            ...
            if not self.swap:
                ...
                temp = [
                    self.swap_video(target_image, frame_number, False),
                    frame_number,
                ]
    ...
    def swap_video(self, target_image, frame_number, change_parameters):
        ...
        if parameters["OrientationState"]:
            img = transforms.functional.rotate(
                img, angle=parameters["OrientationAmount"][0], expand=True
            )

        # Find all faces in frame and return a list of 5-pt kpss
        kpss = self.func_w_test(
            "detect",
            self.detect,
            img,
            input_size=(640, 640),
    ...
    def swap_core(self, img, kps, s_e, parameters, frame):  # img = RGB
        ...
        if parameters["TransformState"]:
            original_face_128 = v2.functional.affine(
                original_face_128,
                0,
                (0, 0),
                1 + parameters["TransformAmount"][0] / 100,
                0,
                center=(63, 63),
                interpolation=v2.InterpolationMode.BILINEAR,
            )
        ...
        if parameters["StrengthState"]:
            itex = ceil(parameters["StrengthAmount"][0] / 100.0)
        ...
        for i in range(itex):
            ...
            io_binding.bind_output(
                ...
                device_id=0,
                element_type=np.float32,
                shape=tuple(swap.shape),
                buffer_ptr=swap.data_ptr(),
            )
        ...
        if parameters["FaceParserState"]:
            mask = self.func_w_test(
                "bg parser",
                self.apply_bg_face_parser,
                swap,
                parameters["FaceParserAmount"][1],
            )
            mask2 = self.func_w_test(
                "mouth parser",
        ...
        gauss = transforms.GaussianBlur(
            parameters["BlurAmount"][0] * 2 + 1, (parameters["BlurAmount"][0] + 1) * 0.2
        )
        ...
        if parameters["ColorState"]:
            ...
            del_color = torch.tensor(
                [
                    parameters["ColorAmount"][0],
                    parameters["ColorAmount"][1],
                    parameters["ColorAmount"][2],
                ],
                device=device,
            )
            swap += del_color
    ...
    def apply_face_parser(self, img, FaceParserAmount):
        ...
        if FaceParserAmount != 0:
            ...
            outpred = torch.empty(
                (1, 19, 512, 512), dtype=torch.float32, device="cuda"
            ).contiguous()
    ...
    def apply_bg_face_parser(self, img, FaceParserAmount):
        ...
        if FaceParserAmount != 0:
            ...
            img = torch.reshape(img, (1, 3, 512, 512))
            outpred = torch.empty(
                (1, 19, 512, 512), dtype=torch.float32, device=device
            ).contiguous()
