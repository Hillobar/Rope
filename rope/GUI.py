...
class GUI(tk.Tk):
    def __init__(self):
        ...
        self.frame_style = {"bg": "gray20", "relief": "flat", "bd": "0"}
        self.checkbox_style = {
            "bg": "gray40",
            "fg": "white",
            "relief": "flat",
        ...
        # Video button canvas
        self.video_button_canvas = tk.Canvas(
            self.media_control_canvas, self.canvas_style1, width=112, height=40
        )
        ...
        # Video Slider canvas
        self.video_slider_canvas = tk.Canvas(
            self.media_control_canvas, self.canvas_style1, height=40
        )
        self.video_slider_canvas.grid(row=0, column=1, sticky="NEWS", pady=0)

        # Video Slider
        self.video_slider = tk.Scale(
            self.video_slider_canvas, self.slider_style, orient="horizontal"
        )
        self.video_slider.bind(
            "<B1-Motion>",
            lambda event: self.slider_move("motion", self.video_slider.get()),
        )
        self.video_slider.bind(
            "<ButtonPress-1>",
            lambda event: self.slider_move("press", self.video_slider.get()),
        )
        self.video_slider.bind(
            "<ButtonRelease-1>",
            lambda event: self.slider_move("release", self.video_slider.get()),
        )
        self.video_slider.bind(
            "<ButtonRelease-3>",
            lambda event: self.slider_move("motion", self.video_slider.get()),
        ...
        self.video_slider.bind(
            "<MouseWheel>",
            lambda event: self.mouse_wheel(event, self.video_slider.get()),
        )
        self.video_slider.pack(fill=tk.X)

        # Marker canvas
        self.marker_canvas = tk.Canvas(
            self.media_control_canvas, self.canvas_style1, width=180, height=40
        )
        ...
        # Label Frame 1
        self.label_frame1 = tk.LabelFrame(
            self.options_frame_canvas1, self.frame_style, height=71, width=1400
        )
        ...
        self.CLIP_text = tk.Entry(
            self.label_frame1, relief="flat", bd=0, textvariable=self.temptkstr
        )
        ...
        # Canvas
        self.found_faces_buttons_canvas = tk.Canvas(
            self.found_faces_frame, self.canvas_style1, height=100, width=112
        )
        self.found_faces_buttons_canvas.grid(
            row=0,
            column=0,
        )
        ...
        self.source_button_canvas = tk.Canvas(
            self.source_faces_frame, self.canvas_style1, height=100, width=112
        )
        ...
        self.source_faces_canvas = tk.Canvas(
            self.source_faces_frame, self.canvas_style1, height=100
        )
        ...
        self.target_button_canvas = tk.Canvas(
            self.target_videos_frame, self.canvas_style1, height=100, width=112
        )
        ...
        self.target_media_canvas = tk.Canvas(
            self.target_videos_frame, self.canvas_style1, height=100
        )
        ...
        # Left Canvas
        self.program_options_frame_canvas = tk.Canvas(
            self.program_options_frame, self.canvas_style1, height=42
        )
        self.program_options_frame_canvas.grid(row=0, column=0, sticky="NEWS", pady=0)

        # Label Frame 1
        self.program_options_label = tk.LabelFrame(
            self.program_options_frame_canvas,
            self.frame_style,
            height=42,
            width=800,
        )
        ...
        column = 8
        x_space = 40
        self.create_ui_button_2(
            "StartRope",
            self.program_options_label,
            column,
            8,
            width=125,
            height=26,
        ...
        self.donate_label.bind(
            "<Button-1>",
            lambda e: self.redirection_callback(
                "https://www.paypal.com/donate/?hosted_button_id=Y5SB9LSXFGRF2"
            ),
        )

    def redirection_callback(self, url):
    ...
    def key_event_handler(self, event):
        """Handle key presses for video player?"""
        # print(event.char, event.keysym, event.keycode)

        # ignore keys in these conditions
        if (
            self.focus_get() == self.CLIP_name
            or self.focus_get() == self.me_name
            or self.actions["ImgVidMode"] != 0
        ):
            return
        ...
        # other keys
        offsets = {"w": 1, "s": -1, "d": 30, "a": -30}
        if event.char not in offsets.keys():
            return

        next_frame = self.video_slider.get() + offsets[event.char]
        next_frame = max(0, next_frame)
        next_frame = min(next_frame, self.video_length)

        self.video_slider.set(next_frame)
        self.add_action("get_requested_video_frame", next_frame)
        self.parameter_update_from_marker(next_frame)
    ...
    def initialize_gui(self):
        ...
        try:
            ...
            if self.json_dict["source videos"] == None:
                self.actions["LoadTVideosButton"].configure(
                    self.button_highlight_style, text=" Setup"
                )
            else:
        ...
        try:
            ...
            if self.json_dict["source faces"] == None:
                self.actions["LoadSFacesButton"].configure(
                    self.button_highlight_style, text=" Setup"
                )
            else:
        ...
        try:
            ...
            if self.json_dict["saved videos"] == None:
                self.actions["OutputFolderButton"].configure(
                    self.button_highlight_style, text=" Setup"
                )
            else:
        ...
        self.bind("<Key>", lambda event: self.key_event_handler(event))
        self.bind("<space>", lambda event: self.key_event_handler(event))

        self.undock_video_window.bind("<Key>", lambda event: self.key_event_handler(event))
        self.undock_video_window.bind("<space>", lambda event: self.key_event_handler(event))
        ...
        self.undock_video_window.geometry(
            "%dx%d+%d+%d"
            % (
                self.json_dict["player_geom"][0],
                self.json_dict["player_geom"][1],
                self.json_dict["player_geom"][2],
            ...
            try:
                ...
                with open("merged_embeddings.txt", "r") as embedfile:
                    ...
                    for i in range(0, len(temp), 513):
                        to = [
                            temp[i][6:],
                            np.array(temp[i + 1 : i + 513], dtype="float32"),
                        ]
                        temp0.append(to)
                ...
                for j in range(len(temp0)):
                    ...
                    self.source_faces[j]["TKButton"].bind(
                        "<ButtonRelease-1>",
                        lambda event, arg=j: self.toggle_source_faces_buttons_state(event, arg),
                    )
                    self.source_faces[j]["TKButton"].bind(
    ...
    def toggle_found_faces_buttons_state(self, button):
        ...
        for i in range(len(self.target_faces[button]["SourceFaceAssignments"])):
            ...
            self.source_faces[self.target_faces[button]["SourceFaceAssignments"][i]][
                "TKButton"
            ].config(self.button_highlight_style)
            ...
            for i in range(len(images)):
                ...
                self.target_media_buttons[i].config(
                    ...
                    command=lambda i=i: self.load_target(
                        i,
                        images[i][1],
                        self.actions["ImgVidModes"][self.actions["ImgVidMode"]],
                    ),
                )
    ...
    def redisplay_markers(self):
        ...
        for i in range(len(self.markers)):
            position = 15 + int(
                width * self.markers[i]["frame"] / self.video_slider.configure("to")[4]
            )
            self.video_slider_canvas.delete(self.markers[i]["icon_ref"])
            self.markers[i]["icon_ref"] = self.video_slider_canvas.create_image(
                position, 30, image=self.marker_icon
            )
    ...
    def check_for_video_resize(self):
        if self.docked:
            ...
            # Read the geometry from the last time json was updated. json only updates once the window ahs stopped changing
            win_geom = "%dx%d+%d+%d" % (
                self.json_dict["undock_win_geom"][0],
                self.json_dict["undock_win_geom"][1],
                self.json_dict["undock_win_geom"][2],
                self.json_dict["undock_win_geom"][3],
            )
    ...
    def add_action(
        ...
        action,
        parameter=None,
        request_updated_frame=False,
        ignore_markers=True,
    ):
        self.action_queue.append([action, parameter])

        if not self.play_video and request_updated_frame:
            if self.actions["ImgVidMode"] == 0 and not ignore_markers:
                self.action_queue.append(["get_requested_video_frame", self.video_slider.get()])
            else:
                self.action_queue.append(
                    [
                        "get_requested_video_frame_parameters",
                        self.video_slider.get(),
                    ]
                )
    ...
    def toggle_stop(self):
        if self.stop_marker == self.video_slider.get():
            ...
            width = self.video_slider_canvas.winfo_width() - 30
            position = 15 + int(
                width * self.video_slider.get() / self.video_slider.configure("to")[4]
            )
            self.stop_image = self.video_slider_canvas.create_image(
                position, 30, image=self.stop_marker_icon
            )
    ...
    def create_ui_button(self, parameter, root, x, y, width=125, height=26):
        ...
        self.param_const[button] = tk.Button(
            root,
            self.inactive_button_style,
            compound="left",
            image=self.param_const[icon_holder],
            anchor="w",
        ...
        self.param_const[button].bind(
            "<ButtonRelease-3>",
            lambda event: self.cycle_ui_button_modes(parameter),
        )

        # Mousewheel function - adjust parameter
        self.param_const[button].bind(
            "<MouseWheel>",
            lambda event: self.update_parameter_data(event, parameter),
        )

        # Status Text
        self.param_const[button].bind(
            "<Enter>", lambda event: self.set_status(self.parameters[message])
        )
    ...
    def create_ui_button_2(self, parameter, root, x, y, width=96, height=26):
        ...
        if parameter == "Dock":
            ...
            self.actions[icon_holder] = ImageTk.PhotoImage(resized_image)
            self.actions[button] = tk.Button(
                root,
                self.inactive_button_style,
                compound="left",
                image=self.actions[icon_holder],
                anchor="w",
        ...
        if parameter == "ImgDock":
            ...
            self.actions[button] = tk.Button(
                ...
                image=self.actions[icon_holder],
                anchor="w",
                command=lambda: self.toggle_auto_swap(),
            )
        ...
        # self.actions[button].bind("<MouseWheel>", self.change_video_quality)
            ...
            self.actions[button] = tk.Button(
                ...
                self.inactive_button_style,
                compound="left",
                text=self.actions["PerfTestModes"][0],
                image=self.actions[icon_holder],
                anchor="w",
                command=lambda: self.toggle_perf_test(),
            )
            ...
            self.actions[button] = tk.Button(
                ...
                self.inactive_button_style,
                compound="left",
                text=self.actions["ClearmemModes"][0],
                image=self.actions[icon_holder],
                anchor="w",
                command=lambda: self.clear_mem(),
            )
